...
async function getAnalysis(req, res) {
  try {
    ...
    if (isPostgres()) {
      const [salesRes, expRes, itemsRes, probRes] = await Promise.all([
    ...
    const analysis = calculateFinancialMetrics(sales, expenses, items, problems);
    ...
    return res.status(500).json({ error: 'Failed to run financial analysis', details: err.message });
  }
}
